...
def test_ask_no_context_escalates_without_llm(client, monkeypatch):
    ...
    r = client.post("/api/chatbot/ask", json={"message": "question inconnue"})
    assert r.status_code == 200
    body = r.json()
...
def test_ask_with_context_returns_llm_answer(client, monkeypatch):
    ...
    r = client.post("/api/chatbot/ask", json={"message": "comment rejoindre un club ?"})
    assert r.status_code == 200
    body = r.json()
...
def test_ask_human_keyword_overrides_escalation(client, monkeypatch):
    ...
    r = client.post("/api/chatbot/ask", json={"message": "je veux un humain"})
    assert r.status_code == 200
